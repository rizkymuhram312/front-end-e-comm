...
const VendorRulesTable = (props) => {
    ...
    let [veru_vendor_name,setVeruName]= useState(props.name)
    let [veru_bill_amount,setVeruAmount]= useState(props.amount)
    let [veru_bill_price,setVeruPrice] = useState(props.price)
    let [veru_desc, setVeruDesc] = useState(props.desc)
    ...
    const removeVeru = ()=>{
        ...
        deleteVeru(props.id).then(response=>{
    ...
    const updateVendor = async (veru) => {
        console.log('Params : '+props.id);
        try {
            let result = await axios.put(`${apiTopup}/vendorRules/updateVendorRules/${props.id}`,{
              data : veru
            })
            console.log(result);
      ...
      const deleteVeru = async (id) => {
        try {
            let response = await axios.delete(`${apiTopup}/vendorRules/deleteVendorRules/${id}`)
                            ...
                            <div className="relative w-5/12 my-6 mx-auto">
                                ...
                                <div className="border-0 rounded-lg shadow-lg relative flex flex-col w-full bg-white outline-none focus:outline-none">
                                    ...
                                    <div className="relative p-6 flex-auto">
                                        ...
                                        <div class="mb-3 pt-0">
                                            <input type="text" name="veru_vendor_name" value={veru_vendor_name} onChange={onChangeVeruName} class="select-all px-3 py-3 placeholder-gray-400 text-gray-700 relative bg-white bg-white rounded text-sm shadow outline-none focus:outline-none focus:shadow-outline w-full" />
                                        </div>
