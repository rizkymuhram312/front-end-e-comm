...
const VerifyPayment = (props) => {
  ...
  const onSubmit = (e) => {
    ...
    axios
      ...
      .then((result) => {
        console.log(result);
        props.setWatrNumber(result.data.watr_numbers)
          ...
          <main className="mb-auto h-10 ">
            <form onSubmit={onSubmit}>
              <input
                type="password"
                value={pin}
                onChange={onHandlePinInputChange}
                name="pin"
                placeholder="WALLET PIN NUMBER"
                className="w-2/12 p-3 mt-5 shadow-lg bg-white rounded-xl mr-2 focus:outline-none focus:ring-2 font-light"
              ></input>
              <button
                type="submit"
                className="py-2 px-4 font-extralight text-white rounded-xl bg-blue-500 hover:bg-blue-800 focus:outline-none focus:ring-2 focus:ring-purple-600 focus:ring-opacity-50"
              >
                VERIFY
              </button>
            </form>
            <h1
              ...
              style={{
                visibility:
                  props.verified || props.verified === null
                    ? "hidden"
                    : "visible",
              }}
            >
              PIN SALAH, SILAHKAN INPUT KEMBALI
            </h1>
          </main>
