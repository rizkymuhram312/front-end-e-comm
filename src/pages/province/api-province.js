...
const listProvince = async () => {
    try {
        let response = await axios.get("http://localhost:3001/api/province/")
...
const create = async (province) => {
    try {
        let response = await axios.post("http://localhost:3001/api/province/",{
          data : province
        })
        return await response.data
      } catch(err) {
  ...
  const deleteProvince = async (provinceId) => {
    try {
        let response = await axios.delete(`http://localhost:3001/api/province/${provinceId}`)
        return await response.data
      } catch(err) {
        return await err.message
      }
  ...
  const updateProvince = async (province) => {
    try {
        let response = await axios.put(`http://localhost/api/province/${province.prov_id}`,{
          data : province
        })
        return await response.data
      } catch(err) {
